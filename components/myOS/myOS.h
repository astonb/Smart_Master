...
#ifdef MCU_ESP8266
#define TaskENTER_CRITICAL(MUX) taskENTER_CRITICAL()
#define TaskEXIT_CRITICAL(MUX)  taskEXIT_CRITICAL()
#endif

//此处定义的优先级数值越大,优先级越低（与FreeRtos相反）
#define OS_APP_PRIORITY (5)
#define OS_SEND_PRIORITY (4)
#define OS_REC_PRIORITY (3)
#define OS_EXEC_PRIORITY (2)

#define OS_WAIT_FOREVER portMAX_DELAY


typedef uint32_t os_thread_arg_t;
typedef void *os_thread_t;
typedef void (*os_thread_function_t)(os_thread_arg_t arg);
...
void my_rtos_set_semaphore_ISR(os_semaphore_t *semaphore);
